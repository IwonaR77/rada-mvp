Wersja promptu: 2 (2026-08-23 — schemat bloku stanu zsynchronizowany z Prompt_Profil_Radnego_Iteracyjny_v2: dodano `po_mowil_o`, uproszczono `udzial_forma.przyklady` na `{opis, sesja, kotwica}`)

---

**Cel tego promptu:** wygenerować profil radnego z całej kadencji jednym przebiegiem (jak dzisiejszy Prompt_Ocena_Radnych_v6), ale zwrócić notatkę **razem ze** strukturalnym stanem (`tematy`/`udzial_forma`/`mieszkancy`/`interpelacje_powiazane`/`spory`) w tym samym formacie, co Prompt_Profil_Radnego_Iteracyjny_v2 — po to, żeby dało się porównać ten baseline z wynikiem ścieżki iteracyjnej (sesja-po-sesji) tą samą miarą, a nie „na oko".

Poza tym dodatkiem — identyczne zasady i format notatki co Prompt_Ocena_Radnych_v6.
...
- Formatowanie notatki: tylko pogrubienia (`**...**`) i zwykłe zdania lub listy punktowane (`- `). Bez tabel, cytatów, bloków kodu, linków.

---

DODATKOWO — blok stanu (dla porównania z eksperymentem iteracyjnym):

Po notatce dołącz **jeden** blok ```json ze strukturalnym stanem, w dokładnie tym samym schemacie co `Prompt_Profil_Radnego_Iteracyjny_v2.md`:
...
```json
{
  ...
  "kategorie_znane": ["…"],
  "tematy": [ { "id": "t1", "teza": "…", "po_mowil_o": "…", "kategoria_obszaru": "…", "sesje": ["…"], "wystapien": N, "pierwsza": "…", "ostatnia": "…", "kotwica": { "sesja": "…", "cytat": "…" }, "sprawa": null, "rola_w_sprawie": null } ],
...
Zbuduj go **z całego materiału naraz** (nie sesja-po-sesji, bo tu widzisz wszystko jednocześnie) — ale każdy `id` tematu/sporu/itd. ma być unikalny, `kotwica` dosłowna (≤120 znaków) i pochodzić z sesji wymienionej przy tym wpisie. `tematy[].wystapien` i `sesje` mają odzwierciedlać rzeczywistą liczbę i daty wystąpień tego tematu w całym materiale, nie tylko w ostatniej sesji, w której się pojawił. Każdy temat dostaje `kategoria_obszaru` (szeroka kategoria tematyczna, np. „infrastruktura drogowa", „budżet i finanse", „oświata") — użyj tej samej nazwy dla tematów należących do tej samej kategorii, zbierz użyte nazwy w `kategorie_znane`. Pola „Główne obszary zainteresowania" i „temat wracający" **nie** mają odpowiednika osobnego pola w stanie — są liczone z `tematy` przez kod, dokładnie jak w wersji iteracyjnej; nie musisz dbać o ich spójność z prozą notatki wyżej poza tym, że oba mają wynikać z tych samych `tematy`.

**`teza` vs `po_mowil_o`** — `teza` to etykieta tematu w mianowniku (kod używa jej dosłownie jako tytułu w cudzysłowie i do dopasowywania tematów). `po_mowil_o` to ta sama treść odmieniona w miejscowniku, gotowa do dokończenia zdania „Mówił o ___" — te same słowa, wyłącznie inny przypadek, bez parafrazy i skracania. Przykład: `teza: "rozbiórka budynków przy ul. Mogielnickiej 6 i 8"` → `po_mowil_o: "rozbiórce budynków przy ul. Mogielnickiej 6 i 8"`.

`udzial_forma.przyklady` — obiekty `{opis, sesja, kotwica}`. `opis` to krótkie określenie czynności w mianowniku/rzeczownikowo, nie pełne zdanie (np. „odczytanie projektu uchwały ws. skargi").
