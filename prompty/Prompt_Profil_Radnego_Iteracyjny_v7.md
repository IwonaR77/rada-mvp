Wersja promptu: 7 (2026-08-23 — `udzial_forma` rozdzielone z 3 na 4 kategorie: `prowadzenie` (prowadzenie obrad) wydzielone z dawnej `czynność formalna lub prowadzenie obrad`. Prowadzenie obrad — otwarcie/zamknięcie dyskusji, zarządzenie głosowania, udzielanie głosu — to w praktyce czynność zarezerwowana dla przewodniczącej/ego rady i wiceprzewodniczących w zastępstwie; zwykły radny jej nie wykonuje. Łączenie jej z generycznymi czynnościami formalnymi (zgłoszenie kandydatury, wniosek formalny, przedstawienie stanowiska komisji) pod jedną etykietą zacierało tę różnicę i ryzykowało niewłaściwe sklasyfikowanie wypowiedzi zwykłego radnego)
...
```json
{
  ...
  "udzial_forma": {
    ...
    "formalna":     { "wystapil": true, "wystapien": 2, "przyklady": [ { "opis": "zgłoszenie kandydatury do komisji", "sesja": "2024-05-13", "kotwica": "…" } ] },
...
**`udzial_forma` (dokładnie cztery klucze: `odczytanie`, `formalna`, `prowadzenie`, `dyskusja`, nigdy więcej/mniej):**
...
- **`prowadzenie`** — czynności osoby FAKTYCZNIE prowadzącej sesję w danym momencie: otwarcie/zamknięcie dyskusji, zarządzenie głosowania, udzielanie głosu innym, stwierdzenie kworum. **To w praktyce funkcja przewodniczącej/ego rady i wiceprzewodniczących w zastępstwie — dla zdecydowanej większości radnych ta kategoria zostanie pusta (`wystapil: false`) przez całą kadencję, i to jest poprawny, oczekiwany wynik, nie błąd.** Rozstrzygaj po treści wypowiedzi w danym momencie, nie po samej funkcji: przewodniczący też czyta uchwały (wtedy `odczytanie`) i zabiera głos merytorycznie (wtedy `dyskusja`) — tylko gdy faktycznie kieruje przebiegiem obrad w tym fragmencie, trafia to tutaj.
- **`dyskusja`** — pytanie, argument, stanowisko wobec sprawy, odpowiedź na cudzy argument.

Zasady wspólne dla wszystkich czterech:
- Gdy w tej sesji wystąpiła dana forma po raz pierwszy → `wystapil: true`.
- **`wystapien`** — licznik osobny od `przyklady`, **nie ograniczony do 3**: zliczaj **każde** wystąpienie tej formy w tej sesji (np. trzy różne pytania w dyskusji na jednej sesji to +3 do `dyskusja.wystapien`, nie +1). To jedyne źródło proporcji między formami w renderze.
...
- **Jeśli poprzedni stan ma `udzial_forma` bez pola `wystapien`** (starszy schemat) — zainicjuj je: `1`, jeśli `wystapil: true`, inaczej `0`, i licz dalej od tej sesji normalnie.
- **Jeśli poprzedni stan ma tylko trzy klucze w `udzial_forma`** (starszy schemat, sprzed rozdzielenia `prowadzenie`) — dodaj czwarty klucz `prowadzenie: {"wystapil": false, "wystapien": 0, "przyklady": []}`. Dodatkowo: przejrzyj istniejące `formalna.przyklady` — jeśli któryś opisuje faktyczne prowadzenie obrad (otwarcie/zamknięcie dyskusji, zarządzenie głosowania, udzielanie głosu innym), przenieś go razem z jego `kotwica` do `prowadzenie` i odpowiednio przesuń liczniki `wystapien` między obiema kategoriami. Jeśli nie masz pewności, zostaw w `formalna`.
