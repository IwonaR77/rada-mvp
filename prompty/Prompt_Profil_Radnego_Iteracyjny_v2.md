Wersja promptu: 2 (2026-08-23 — dodano pole `po_mowil_o` (teza odmieniona w miejscowniku, do zdania „Mówił o…"), bo `teza` służy też jako etykieta w Mianowniku w sekcji „Powroty" i jedno pole nie może gramatycznie pasować do obu ról naraz; uproszczono schemat `udzial_forma.przyklady` z `{sesja, punkt, kotwica}` na `{opis, sesja, kotwica}` — oba dotychczasowe przebiegi (jednorazowy i iteracyjny) niezależnie od siebie próbowały zwrócić coś bliższego temu kształtowi zamiast żądanego, co był sygnał, że poprzedni schemat był dla modelu nienaturalny)
...
**`teza` vs `po_mowil_o` — dlaczego dwa pola.** `teza` to etykieta tematu w mianowniku (np. „oświetlenie uliczne w sołectwach") — kod używa jej dosłownie jako tytułu w cudzysłowie w sekcji „Powroty do tematów" i do dopasowywania tematów między wersjami profilu, więc **musi** zostać w mianowniku. `po_mowil_o` to **ta sama treść, odmieniona w miejscowniku**, gotowa do dokończenia zdania „Mówił o ___" — kod wstawia ją bez żadnej dalszej obróbki gramatycznej. Te same słowa, ta sama treść, wyłącznie inny przypadek — nie parafrazuj, nie skracaj, nie dodawaj niczego w `po_mowil_o`, czego nie ma w `teza`. Przykład: `teza: "rozbiórka budynków przy ul. Mogielnickiej 6 i 8"` → `po_mowil_o: "rozbiórce budynków przy ul. Mogielnickiej 6 i 8"`.
...
- Nowy temat, którego nie ma na liście → nowy wpis, `wystapien: 1`, `sesje: [ta data]`, `pierwsza = ostatnia = ta data`, `kotwica` = dosłowny cytat ≤120 znaków **z tej sesji**, `kategoria_obszaru` wg zasady z `kategorie_znane` wyżej, `po_mowil_o` wg zasady wyżej (ustalasz raz, przy tworzeniu tematu). Bez kotwicy nie dodawaj tematu — jeśli sprawa wynika tylko z DANYCH (6) bez wypowiedzi, `kotwica` może wskazywać na treść sprawy zamiast cytatu (zaznacz `"kotwica": null` i uzupełnij `sprawa`/`rola_w_sprawie` — to wystarczający dowód, sprawa istnieje w bazie niezależnie od transkrypcji).
- Ten sam temat pojawia się ponownie (ten sam przedmiot, nie samo podobieństwo słów — dwie wzmianki o „drodze" to jeden temat tylko gdy chodzi o tę samą drogę) → dopisz datę do `sesje` (jeśli jeszcze jej nie ma), `wystapien` += liczba nowych wystąpień w tej sesji, `ostatnia` = ta data, zaktualizuj `kotwica` na cytat z tej (najnowszej) sesji. `teza` i `po_mowil_o` **zostają, jak były** — nie przepisujesz ich przy każdym powrocie do tematu, tylko przy pierwszym utworzeniu (chyba że poprawiasz drobną pomyłkę sprzed sesji).
...
- **Limit 25 tematów**: gdy przekroczysz, scal dwa najbliższe znaczeniowo w jeden (sumując `wystapien`, łącząc `sesje`, biorąc wcześniejszą `pierwsza` i późniejszą `ostatnia`, oraz nową parę `teza`/`po_mowil_o` obejmującą oba scalone tematy) — nigdy nie ucinaj po prostu najstarszego wpisu.
