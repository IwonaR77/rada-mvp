Wersja promptu: 7 (2026-08-10 — nazwa rady, numer sesji i identyfikator pliku pochodzą z nagłówka transkrypcji, a nie z zaszytej w prompcie nazwy jednej rady; opisano dodatkowe metadane wariantu z mówcami)
...
rada: Rada Miejska w Grójcu
numer sesji: XXXII (32 w tej kadencji)
identyfikator: sesja_86312_2026-06-25
data: 2026-06-25
tytuł: Sesja Rady w dniu czwartek, 25 czerwca 2026
tagi: budżet, oświata, transport, infrastruktura drogowa, ...

**Wszystkie te dane weź z nagłówka, nie pytaj mnie o nie** — są już
w treści, którą wklejam. Zapytaj tylko, jeśli naprawdę ich tam nie ma
(np. wkleiłam fragment bez nagłówka).

Serwis obsługuje **więcej niż jedną radę** (rada gminy, rada powiatu),
dlatego linia `rada:` jest wiążąca: nazwy rady nigdy nie zgaduj ani nie
przepisuj z przykładów w tym prompcie. Ta sama zasada dotyczy linii
`numer sesji:` — tytuł sesji bywa go pozbawiony, a nagłówek nie.

Linia `tagi:` to **aktualna lista wszystkich tagów tematycznych użytych
dotąd w innych sesjach tej rady** — patrz sekcja TAGOWANIE niżej, to z niej
korzystasz przy wyborze tagów dla tej sesji. Jeśli w nawiasie widnieje
informacja, że tagów jeszcze nie ma, dobierasz je od zera.

Jeżeli wkleiłam wariant transkrypcji **z mówcami**, nagłówek ma dodatkowo:

skład rady: Jan Madej (przewodniczący), Anna Kowalska, ...
urzędnicy: Adam Nowak (skarbnik), ...
otagowani mówcy: 812 z 1267 wypowiedzi (64%)

Imiona i nazwiska w sekcji SPORY I DYSKUSJE bierz **w brzmieniu ze składu
rady i listy urzędników**, a nie ze słuchu transkrypcji — transkrypcja
automatyczna nagminnie przekręca nazwiska. Linia `otagowani mówcy:` mówi,
na ile atrybucja wypowiedzi jest kompletna: przy niskim odsetku etykieta
„Nieustalone" dominuje i **nie wolno na siłę przypisywać wypowiedzi
konkretnym osobom** — wtedy opisuj spór bez wskazywania nazwisk, których
transkrypcja nie potwierdza.

NAZEWNICTWO PLIKU:

Nazwa pliku wynikowego to zawsze `<identyfikator>_podsumowanie.md`, gdzie `<identyfikator>` przepisujesz dosłownie z linii `identyfikator:` w nagłówku transkrypcji. To ten sam identyfikator, co pliki wideo/VTT w pipeline transkrypcji i pobrana transkrypcja (`<identyfikator>_transkrypcja.txt`), plus przyrostek `_podsumowanie`, żeby oba pliki leżące obok siebie w folderze dało się odróżnić na pierwszy rzut oka, a nie dopiero po rozszerzeniu. Przykład: dla `identyfikator: sesja_86312_2026-06-25` plik nazywa się `sesja_86312_2026-06-25_podsumowanie.md`. Nie składaj tej nazwy samodzielnie z daty ani z numeru sesji, nie używaj cyfr rzymskich ani nazwy rady — te informacje są już w treści dokumentu, nazwa pliku ma być tylko unikalnym, sortowalnym identyfikatorem.
...
```
## Sesja [numer rzymski z linii „numer sesji:" nagłówka] [nazwy rady z linii „rada:" nagłówka, w dopełniaczu — np. „Rady Miejskiej w Grójcu", „Rady Powiatu Grójeckiego"] ([data sesji])
