Wersja promptu: 8 (2026-09-06 — bez zmian w tej połowie pary: seed nie ma poprzedniego stanu do porównania, więc problem "model re-generuje cały stan" go nie dotyczy. Podbite tylko dla spójności numeracji z Prompt_Profil_Radnego_Iteracyjny_v8, z którym ten plik jest sklejany w jeden dokument (`src/lib/councilor-profile-prompt.ts`). Schemat `stan` (SCHEMAT STANU niżej) jest identyczny jak w wersji 7 — `wersja_stanu` w przykładzie zostaje `7`, bo to pole opisuje schemat PERSYSTOWANEGO stanu, nie wersję promptu, a schemat się nie zmienił)

---

**Cel tego promptu:** wygenerować profil radnego z całej kadencji jednym przebiegiem (jak dzisiejszy Prompt_Ocena_Radnych_v6), ale zwrócić notatkę **razem ze** strukturalnym stanem (`tematy`/`udzial_forma`/`mieszkancy`/`interpelacje_powiazane`/`spory`) w tym samym formacie, co Prompt_Profil_Radnego_Iteracyjny_v8 — po to, żeby dało się porównać ten baseline z wynikiem ścieżki iteracyjnej (sesja-po-sesji) tą samą miarą, a nie „na oko".
...
[W przeciwnym razie: **lista punktowana (`- `), jeden punkt na formę, która wystąpiła** (nie akapit prozy). Rozróżniasz cztery formy:
 ...
 (b) **czynność formalna** — dostępna każdemu radnemu: zgłoszenie wniosku formalnego, zgłoszenie kandydatury, ślubowanie, wniosek o zmianę zapisu/porządku obrad, przedstawienie stanowiska lub opinii komisji własnymi słowami (nie dosłowny cytat — to trafia do (a));
 (c) **prowadzenie obrad** — otwarcie i zamknięcie dyskusji, zarządzenie głosowania, udzielanie głosu innym, stwierdzenie kworum. **To w praktyce funkcja przewodniczącej/ego rady i wiceprzewodniczących w zastępstwie** — dla większości radnych ta forma w ogóle nie wystąpi w materiale, i to jest poprawny wynik, nie luka do wypełnienia;
 (d) **głos w dyskusji** — pytanie, argument, stanowisko wobec sprawy, odpowiedź na cudzy argument.
Każdy punkt: nazwa formy, przy jakich punktach porządku obrad wystąpiła, z datą sesji przy przykładach (2–3 przykłady wystarczą, nie wyczerpująca lista). Rozstrzygaj po treści wypowiedzi w danym momencie, nie po funkcji radnego: przewodniczący też czyta uchwały (a) i zabiera głos merytorycznie (d) — do (c) trafia tylko wtedy, gdy w tym fragmencie faktycznie kieruje przebiegiem obrad. Uwaga na częsty przypadek: mówienie **o** paragrafie uchwały ("w paragrafie 4 mamy zapisane, że…") to głos w dyskusji, nie odczytanie — odczytanie odtwarza tekst, dyskusja się do niego odnosi.]
...
Po notatce dołącz **jeden** blok ```json ze strukturalnym stanem, w dokładnie tym samym schemacie (SCHEMAT STANU) co `Prompt_Profil_Radnego_Iteracyjny_v8.md` — ten schemat sam w sobie nie zmienił się względem wersji 7:
...
```json
{
  ...
  "udzial_forma": { "odczytanie": { "wystapil": bool, "wystapien": N, "przyklady": [...] }, "formalna": { "wystapil": bool, "wystapien": N, "przyklady": [...] }, "prowadzenie": { "wystapil": bool, "wystapien": N, "przyklady": [...] }, "dyskusja": { "wystapil": bool, "wystapien": N, "przyklady": [...] } },
