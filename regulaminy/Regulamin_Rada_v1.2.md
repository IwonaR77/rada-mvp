...
4. Przeglądanie treści Serwisu — w tym transkrypcji sesji, spraw, profili radnych i innych treści zweryfikowanych/finalnych — jest publiczne i nie wymaga logowania, także dla ruchu zautomatyzowanego (crawlerów, botów). Logowania wymaga wyłącznie współtworzenie Serwisu, na zasadach opisanych w §3 i §5.
...
- **Użytkownik** — osoba korzystająca z Serwisu, w tym anonimowo przeglądająca jego publicznie dostępne treści lub zalogowana za pośrednictwem konta Google.
...
- **Manager** — Współtwórca posiadający pełny zakres uprawnień administracyjnych w Serwisie, w tym prawo do zarządzania uprawnieniami innych Użytkowników.
...
1. Przeglądanie transkrypcji sesji, spraw, profili radnych oraz pozostałych treści Serwisu o statusie zweryfikowanym/finalnym jest publiczne i nie wymaga zalogowania — dotyczy to również ruchu zautomatyzowanego (botów, crawlerów). Serwis nie stosuje technicznych środków blokujących taki ruch, poza podstawowym ograniczeniem liczby zapytań (rate-limit) na funkcję wyszukiwania. Propozycje przypisań, które nie zostały jeszcze zatwierdzone przez Moderatora, pozostają zastrzeżone dla Użytkowników zalogowanych.
2. Logowania (wyłącznie przez Google OAuth — patrz §4) wymagają działania współtworzące: proponowanie i zatwierdzanie przypisania Segmentu do Osoby wymienionej w Segmencie, pobieranie surowych plików transkryptu (.txt/.srt) oraz dostęp do panelu administracyjnego Serwisu. Wymagają one dodatkowo posiadania odpowiedniego uprawnienia, o którym mowa w §5.
3. Segmenty transkrypcji są przypisywane do osób w dwuetapowym procesie redakcyjnym, a nie w drodze publicznego głosowania społeczności: Współtwórca z uprawnieniem Redaktora **proponuje** przypisanie (Segment otrzymuje status „proposed"), a Współtwórca z uprawnieniem Moderatora **zatwierdza** propozycję albo przypisuje Segment bezpośrednio ze statusem finalnym. Serwis nie prezentuje publicznego, surowego histogramu głosów ani wyniku ważonego jakąkolwiek formą reputacji Użytkowników — mechanizm taki nie jest aktywną funkcją Serwisu.
4. Moderator może finalizować (zamykać) Segmenty — po finalizacji Segment nie podlega dalszym propozycjom ze strony zwykłych Redaktorów. Finalizacja nie jest gwarancją stuprocentowej trafności wskazania osoby wypowiadającej się i może zostać cofnięta w drodze procedury sprostowania opisanej w §8.
5. Serwis obecnie nie posiada wyodrębnionego konta technicznego, którego przypisania generowane byłyby automatycznie przez modele językowe lub inne mechanizmy zautomatyzowane. Wprowadzenie takiej funkcji w przyszłości wymagałoby aktualizacji niniejszego Regulaminu.
6. Wynik przypisania wypowiedzi do Osoby wymienionej w Segmencie ma charakter propozycji lub ustalenia redakcyjnego dokonanego przez człowieka (Redaktora/Moderatora), nie zaś zautomatyzowanego podejmowania decyzji w rozumieniu art. 22 RODO. Wynik ten może zostać w każdej chwili skorygowany w drodze zgłoszenia, o którym mowa w §8, i nie stanowi ostatecznego, wiążącego ustalenia autorstwa wypowiedzi.
...
3. Samo zalogowanie się do Serwisu nie nadaje Użytkownikowi żadnych uprawnień współtworzenia — zasady ich uzyskiwania określa §5.
...
1. Serwis nie posiada trzech płaskich ról „Użytkownik/Moderator/Administrator". Dostęp do Serwisu opiera się na następujących poziomach:
   - **anonimowy dostęp publiczny** — przeglądanie treści bez logowania i bez konta,
   - **konto zalogowane bez dodatkowych uprawnień** — zalogowanie przez Google nie daje automatycznie prawa do współtworzenia,
   - **nadane uprawnienia współtworzenia**, przypisane do konta i opcjonalnie zawężone do konkretnej rady/jednostki terytorialnej: uprawnienie **Redaktora** (proponowanie przypisań) oraz uprawnienie **Moderatora** (dodatkowo: zatwierdzanie/finalizacja przypisań oraz pobieranie surowych transkryptów .txt/.srt),
   - **uprawnienie Managera** — pełny dostęp administracyjny, w tym zarządzanie uprawnieniami innych Użytkowników; nadawane wyłącznie ręcznie przez istniejącego Managera i niepodlegające samodzielnemu wnioskowaniu,
...
5. Manager może samodzielnie, z własnej inicjatywy i bez wniosku Użytkownika, nadać, zmienić lub cofnąć uprawnienia danego Użytkownika, w szczególności w związku z nadużyciami, o których mowa w §6.
6. Manager może zablokować konto Użytkownika w całości, uniemożliwiając mu dalsze korzystanie z Serwisu, w szczególności w przypadkach opisanych w §6. Zablokowanie konta nie jest równoznaczne z usunięciem danych osobowych Użytkownika — zasady przechowywania i usuwania danych określa Polityka Prywatności Serwisu, §6.
...
2. Manager, a w zakresie danej rady także Moderator, może:
   - cofnąć lub ograniczyć uprawnienia Użytkownika dopuszczającego się rażącego, celowego nadużycia,
...
1. Uwagi, zgłoszenia błędnych przypisań, uwagi do publikowanej treści prompta wykorzystywanego w procesie tworzenia Serwisu lub inne reklamacje dotyczące funkcjonowania Serwisu należy kierować na adres: [adres e-mail Administratora].
