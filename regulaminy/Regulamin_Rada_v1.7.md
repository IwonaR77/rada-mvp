...
1. Niniejszy Regulamin określa zasady korzystania z serwisu internetowego „Rada" (dalej: „Serwis"), dostępnego pod adresem ktomowi.pl.
2. Administratorem Serwisu jest osoba fizyczna, kontakt: iwona.rysik@gmail.com (dalej: „Administrator").
...
4. Przeglądanie treści Serwisu — transkrypcji sesji, spraw, profili radnych i pozostałych treści objętych Uprawnieniem przeglądania — nie wymaga zalogowania ani posiadania konta. Zalogowanie przez Google OAuth jest wymagane wyłącznie do działań wykraczających poza samo przeglądanie: współtworzenia treści (proponowanie i zatwierdzanie przypisań Segmentów), wyszukiwania pełnotekstowego w transkrypcjach, zapisywania prywatnych notatek do zakładek oraz dostępu do panelu administracyjnego. Zasady przeglądania bez logowania oraz zakres uprawnień związanych z zalogowaniem określają §3 i §5.
5. Rozpoczęcie korzystania z Serwisu, w tym pierwsze zalogowanie się do Serwisu, oznacza akceptację niniejszego Regulaminu.
6. Serwis, w tym jego kod źródłowy i treści interfejsu, powstaje w całości przy wykorzystaniu narzędzi sztucznej inteligencji (AI) jako wsparcia procesu programistycznego — jedynym elementem tworzonym na tym etapie bezpośrednio przez człowieka są polecenia (prompty) kierowane do tych narzędzi. Nie dotyczy to samego przypisywania wypowiedzi z sesji do Osób wymienionych w Segmencie: zgodnie z §3, jest to ręczny proces redakcyjny wykonywany przez Redaktorów i Moderatorów, nie działanie zautomatyzowane. Zasady publikacji prompta wykorzystywanego do generowania podsumowań sesji i opisów spraw określa §7 ust. 7.

## §2. Definicje

- **Odwiedzający** — osoba korzystająca z Serwisu bez logowania, posiadająca z tego tytułu wyłącznie Uprawnienie przeglądania, na zasadach opisanych w §3.
- **Użytkownik** — osoba posiadająca konto w Serwisie, zalogowana za pośrednictwem konta Google (OAuth).
- **Uprawnienie przeglądania (browse)** — podstawowy, wyłącznie odczytowy poziom dostępu do treści Serwisu, przysługujący każdemu — zarówno Odwiedzającemu bez logowania, jak i każdemu kontu Użytkownika z chwilą pierwszego zalogowania. W odniesieniu do zalogowanego konta jest to odrębne, technicznie nadawane i odwoływalne uprawnienie (nie tylko fakt bycia zalogowanym) — jego cofnięcie przez Managera blokuje dostęp danego konta do treści Serwisu bez usuwania samego konta, na zasadach opisanych w §5. W odniesieniu do Odwiedzającego bez konta Uprawnienie przeglądania wynika wprost z niniejszego Regulaminu i nie jest nadawane ani cofane indywidualnie — ochronę Serwisu przed nadużyciem tego dostępu (np. przez zautomatyzowane pobieranie treści) zapewniają wyłącznie mechanizmy opisane w §3 ust. 8.
...
1. Przeglądanie transkrypcji sesji, spraw, profili radnych oraz pozostałych treści Serwisu objętych Uprawnieniem przeglądania jest możliwe bez zalogowania — Serwis ma w tym zakresie charakter otwarty. Zalogowanie przez Google OAuth (patrz §4) jest wymagane dopiero dla działań i treści wykraczających poza samo przeglądanie, wskazanych w ust. 3 poniżej.
2. Uprawnienie przeglądania przysługuje z mocy niniejszego Regulaminu każdemu Odwiedzającemu bez logowania, a każde konto Użytkownika otrzymuje je dodatkowo i automatycznie z chwilą pierwszego zalogowania, o czym mowa w §2. W przypadku zalogowanego konta cofnięcie tego uprawnienia przez Managera, na zasadach opisanych w §5, blokuje Użytkownikowi dostęp do treści Serwisu bez konieczności usuwania jego konta; wobec Odwiedzającego bez konta taki mechanizm indywidualnego cofnięcia nie istnieje — patrz ust. 8 poniżej.
3. Działania współtworzące — proponowanie i zatwierdzanie przypisania Segmentu do Osoby wymienionej w Segmencie, pobieranie surowych plików transkryptu (.txt/.srt) oraz dostęp do panelu administracyjnego Serwisu — a także wyszukiwanie pełnotekstowe w transkrypcjach oraz zapisywanie prywatnych notatek do zakładek, wymagają zalogowania oraz, poza samym zalogowaniem, posiadania odrębnych, dodatkowych uprawnień, o których mowa w §5, wykraczających poza samo Uprawnienie przeglądania.
...
8. Otwarcie przeglądania treści dla Odwiedzających bez logowania (ust. 1–2) obejmuje również ruch zautomatyzowany (crawlerów, botów) w zakresie, w jakim nie jest on ograniczany przez mechanizmy opisane poniżej — Serwis nie wymaga już logowania jako warunku dostępu, w tym wobec ruchu zautomatyzowanego. Aby ograniczyć nadmierne lub masowe automatyczne pobieranie treści, Serwis stosuje: techniczny limit liczby żądań w jednostce czasu przypisany do adresu IP (patrz Polityka Prywatności, §2 pkt 3) oraz usługę Cloudflare pośredniczącą w ruchu sieciowym do Serwisu, obejmującą ochronę przed botami i nadmiernym ruchem (patrz Polityka Prywatności, §6). Przekroczenie limitu skutkuje czasowym odrzuceniem żądań z danego adresu IP, bez wpływu na konta ani dane innych Odwiedzających czy Użytkowników.
...
3. Uprawnienie przeglądania przysługuje niezależnie od zalogowania (patrz §3 ust. 1–2), dlatego zalogowanie się do Serwisu nie jest do niego potrzebne — skutkuje jedynie automatycznym, dodatkowym przypisaniem tego uprawnienia do konta, o czym mowa w §2 i §3 ust. 2, oraz umożliwia dostęp do funkcji dostępnych wyłącznie po zalogowaniu, wskazanych w §3 ust. 3 (m.in. wyszukiwanie, notatki do zakładek). Samo zalogowanie nie nadaje natomiast żadnych uprawnień współtworzenia — zasady ich uzyskiwania określa §5.
...
1. Serwis nie posiada trzech płaskich ról „Użytkownik/Moderator/Administrator". Dostęp do Serwisu opiera się na następujących poziomach:
   - **Uprawnienie przeglądania (browse)** — przysługujące każdemu Odwiedzającemu bez logowania z mocy Regulaminu oraz nadawane dodatkowo, automatycznie, każdemu kontu z chwilą pierwszego zalogowania (patrz §3 ust. 1–2); jego brak w odniesieniu do zalogowanego konta (wskutek cofnięcia przez Managera) uniemożliwia temu Użytkownikowi przeglądanie treści Serwisu — mechanizm ten nie dotyczy Odwiedzających bez konta,
...
3. Serwis przetwarza dane osobowe Osób wymienionych w Segmencie (radnych, urzędników oraz zaproszonych gości sesji) w zakresie: imienia i nazwiska, treści wypowiedzi wygłoszonej publicznie na sesji oraz wyniku przypisania tej wypowiedzi ustalonego w procesie opisanym w §3. Zakres ten obejmuje również dane wyliczone na podstawie powyższych treści (m.in. wskaźniki frekwencji, statystyki aktywności, syntezy generowane z wykorzystaniem sztucznej inteligencji) oraz — w odniesieniu do kandydatów w wyborach samorządowych, niezależnie od tego, czy uzyskali mandat radnego — dane pochodzące z oficjalnych wyników tych wyborów publikowanych przez Państwową Komisję Wyborczą; szczegółowy opis obu kategorii danych zawiera Polityka Prywatności Serwisu, §4. Podstawą prawną tego przetwarzania jest:
...
| 1.4 | 2026-08-08 | Usunięto niewypełnione miejsce na imię i nazwisko Administratora w §1 ust. 2 — Administrator wskazany jest wyłącznie jako osoba fizyczna z podanym adresem e-mail kontaktowym, bez publikacji imienia i nazwiska. |
| 1.5 | 2026-08-08 | Uzupełniono adres Serwisu (ktomowi.pl) w §1 ust. 1. Usunięto zdanie o niepublikowaniu imienia i nazwiska Administratora w §1 ust. 2 jako zbędne. |
| 1.6 | 2026-08-26 | Doprecyzowano §1 ust. 6: wykorzystanie AI „w całości" dotyczy procesu programistycznego (kod, treści interfejsu) — dodano wprost, że przypisywanie wypowiedzi do Osób wymienionych w Segmencie pozostaje ręcznym procesem redakcyjnym (Redaktor/Moderator), a nie działaniem zautomatyzowanym, zgodnie z §3. Rozszerzono §7 ust. 3 (zakres danych osobowych Osób wymienionych w Segmencie) o dane wyliczone na podstawie treści wypowiedzi (m.in. wskaźniki frekwencji, statystyki aktywności, syntezy generowane z wykorzystaniem AI) oraz o dane kandydatów w wyborach samorządowych pochodzące z oficjalnych danych Państwowej Komisji Wyborczej — obie kategorie faktycznie prezentowane są już w Serwisie (profile radnych, dane wyborcze), ale nie były dotąd ujęte w zakresie opisanym w tym paragrafie. Szczegółowy opis obu kategorii, wraz z pełną podstawą prawną i okresem przechowywania, przeniesiono do Polityki Prywatności Serwisu §4 (aktualizowanej równolegle do wersji 1.8). |
| 1.7 | 2026-08-31 | Dostosowano treść do faktycznie wdrożonego modelu dostępu: przeglądanie treści Serwisu (transkrypcje sesji, sprawy, profile radnych) nie wymaga już logowania ani konta — Uprawnienie przeglądania przysługuje z mocy Regulaminu każdemu Odwiedzającemu, nie tylko zalogowanemu kontu. Zalogowanie przez Google OAuth pozostaje wymagane wyłącznie dla działań wykraczających poza przeglądanie: współtworzenia (propozycje/zatwierdzanie przypisań Segmentów), wyszukiwania pełnotekstowego w transkrypcjach, zapisywania notatek do zakładek oraz panelu administracyjnego. Dodano definicję „Odwiedzający" (§2), zaktualizowano definicję Uprawnienia przeglądania (§2), §1 ust. 4, §3 ust. 1–3, §4 ust. 3 oraz §5 ust. 1 tiret pierwsze. Dodano §3 ust. 8 opisujący mechanizmy ochrony przed nadmiernym ruchem zautomatyzowanym wobec ruchu bez logowania (limit żądań per IP, Cloudflare) — uzupełnienie usunięte w wersji 1.3 (wówczas dotyczyło ruchu, który i tak wymagał logowania; obecnie dotyczy realnie otwartego przeglądania). |
